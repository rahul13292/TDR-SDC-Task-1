...
def get_word_score(word, n):
    ...
    score_comp1=0
    score_comp2=(7*word_length)-3*(n-word_length)
    for i in word:
        for values in SCRABBLE_LETTER_VALUES:
            if i==values:
                score_comp1+=SCRABBLE_LETTER_VALUES[values]
    if (score_comp2>1)==False:   
        score_comp2=1
    else:
        score_comp2=(7*word_length)-3*(n-word_length)
    total_score=(score_comp1)*(score_comp2)
    return total_score
...
def update_hand(hand, word):
    ...
    for i in hand:
        word_dict[i] = hand[i]-word_dict.get(i,0) 
    return word_dict
...
def is_valid_word(word, hand, word_list):
    ...
    if '*' in list_of_word:
        position=list_of_word.index('*')
        for letter in VOWELS:
            list_of_word[position]=letter
            words.append("".join(list_of_word))
    else:
        words.append(word)
   ...
   #for general case
    ...
    for letter,frequency in word_dict.items():
        if frequency>hand.get(letter, 0):
            return False
    return True
#
